...
import ObjectInput from '../ObjectInputWithDisabled'
...
export default class EnvironmentInputItem extends React.Component {
  ...
  handleError = (text = '') => {
    ...
    handleKeyError(text)
    handleInputError(text, this.props.index)
  }
  ...
  render() {
    const { value = {}, onChange, canEditIndex, index } = this.props
    const { keyError } = this.state

    const disabled = canEditIndex.includes(index) ? null : true

    if (value.valueFrom) {
      return this.renderConfigOrSecret(disabled)
    }
