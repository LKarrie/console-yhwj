...
export default class ImageSearch extends Component {
  ...
  getImageDetail = async ({ image, secret, insecure, ...rest }) => {
    ...
    let secretDetail = imageRegistries.find(item => item.value === secret)
    const cluster = get(secretDetail, 'cluster') || propsCluster

    // console.log(`search image ${image}`)
    let result
    if (image.includes('harbor') && !secret) {
      // console.log('use harbor search')
      const reg = new RegExp(`${image}(/)?`)
      const regImage = image.replace(reg, '')
      secretDetail = imageRegistries.find(item => item.url.includes(regImage))
      secret = secretDetail.value
      result = await this.store.getImageDetail({
        cluster,
        namespace,
        image,
        secret,
